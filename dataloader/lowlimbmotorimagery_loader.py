...
Path = '/home/qinyz/Brain_EEGCN/datasets/low-limb-motorimagery-dataset/self_EEG_RAW.mat'
...
def form_onesub_set(subject_num: int, size: int = None, step: int = None) -> Tuple[List[torch.Tensor], List[int]]:
    ...
    for i in range(len(data_dump['s_train']['label'][0][subject_num].squeeze())):
        parsed = torch.from_numpy(data_dump['s_train']['eegdata'][0][subject_num][i][0].T).unfold(1, size, step).permute(1, 0, 2)
    ...
    for i in range(len(data_dump['s_test']['label'][0][subject_num].squeeze())):
        parsed = torch.from_numpy(data_dump['s_test']['eegdata'][0][subject_num][i][0].T).unfold(1, size, step).permute(1, 0, 2)
...
def form_multsub_set(tra_sub_lst: List[int], tes_sub_lst: List[int], size: int = None, step: int = None) -> \
    ...
    for te_sub in tes_sub_lst:
        tra_dat, tra_lab, tes_dat, tes_lab = form_onesub_set(te_sub, size=size, step=step)
