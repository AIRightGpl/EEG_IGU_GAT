...
Path = '/home/uestc/Brain_EEGCN/datasets/raw_format_109_public/'
...
def form_onesub_set(subject_num: int, size: int = None, step: int = None, sequence: bool = False) -> \
    ...
    for i in range(len(data_dump['train'][0]['label'][0].squeeze())):
        ...
        if sequence:
            ...
            train_lab.append(data_dump['train'][0]['label'][0].squeeze()[i])
        else:
    ...
    for i in range(len(data_dump['test'][0]['label'][0].squeeze())):
        ...
        if sequence:
            ...
            test_lab.append(data_dump['test'][0]['label'][0].squeeze()[i])
        else:
...
False) -> Tuple[List[torch.Tensor], List[int]]:
    ...
    for te_sub in tes_sub_lst:
        tr_dat, tr_lab, te_dat, te_lab = form_onesub_set(te_sub, size=size, step=step, sequence=seq)
